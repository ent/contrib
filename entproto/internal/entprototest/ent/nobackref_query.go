...
func (nbq *NoBackrefQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*NoBackref, error) {
	...
	if query := nbq.withImages; query != nil {
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "no_backref_images" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "no_backref_images" returned %v for node %v`, *fk, n.ID)
			}
