...
const (
	...
	TypePortal                 = "Portal"
	TypeServiceWithID          = "ServiceWithID"
...
// ServiceWithIDMutation represents an operation that mutates the ServiceWithID nodes in the graph.
type ServiceWithIDMutation struct {
	config
	op            Op
	typ           string
	id            *int32
	...
	oldValue      func(context.Context) (*ServiceWithID, error)
	predicates    []predicate.ServiceWithID
}

var _ ent.Mutation = (*ServiceWithIDMutation)(nil)

// servicewithidOption allows management of the mutation configuration using functional options.
type servicewithidOption func(*ServiceWithIDMutation)

// newServiceWithIDMutation creates new mutation for the ServiceWithID entity.
func newServiceWithIDMutation(c config, op Op, opts ...servicewithidOption) *ServiceWithIDMutation {
	m := &ServiceWithIDMutation{
		config:        c,
		op:            op,
		typ:           TypeServiceWithID,
		clearedFields: make(map[string]struct{}),
	}
	for _, opt := range opts {
		opt(m)
...
// withServiceWithIDID sets the ID field of the mutation.
func withServiceWithIDID(id int32) servicewithidOption {
	return func(m *ServiceWithIDMutation) {
		var (
			err   error
			once  sync.Once
			value *ServiceWithID
		)
		m.oldValue = func(ctx context.Context) (*ServiceWithID, error) {
			once.Do(func() {
				if m.done {
					err = errors.New("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().ServiceWithID.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withServiceWithID sets the old ServiceWithID of the mutation.
func withServiceWithID(node *ServiceWithID) servicewithidOption {
	return func(m *ServiceWithIDMutation) {
		m.oldValue = func(context.Context) (*ServiceWithID, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// Client returns a new `ent.Client` from the mutation. If the mutation was
// executed in a transaction (ent.Tx), a transactional client is returned.
func (m ServiceWithIDMutation) Client() *Client {
	client := &Client{config: m.config}
	client.init()
	return client
}
...
// Tx returns an `ent.Tx` for mutations that were executed in transactions;
// it returns an error otherwise.
func (m ServiceWithIDMutation) Tx() (*Tx, error) {
	if _, ok := m.driver.(*txDriver); !ok {
		return nil, errors.New("ent: mutation is not running in a transaction")
	}
	tx := &Tx{config: m.config}
...
// SetID sets the value of the id field. Note that this
// operation is only accepted on creation of ServiceWithID entities.
func (m *ServiceWithIDMutation) SetID(id int32) {
	m.id = &id
}

// ID returns the ID value in the mutation. Note that the ID is only available
// if it was provided to the builder or after it was returned from the database.
func (m *ServiceWithIDMutation) ID() (id int32, exists bool) {
	if m.id == nil {
		return
	}
	return *m.id, true
...
// as sql.LevelSerializable, the returned ids match the ids of the rows that will be updated
// or updated by the mutation.
func (m *ServiceWithIDMutation) IDs(ctx context.Context) ([]int32, error) {
	switch {
	case m.op.Is(OpUpdateOne | OpDeleteOne):
		id, exists := m.ID()
		if exists {
		...
		fallthrough
	case m.op.Is(OpUpdate | OpDelete):
		return m.Client().ServiceWithID.Query().Where(m.predicates...).IDs(ctx)
	default:
		return nil, fmt.Errorf("IDs is not allowed on %s operations", m.op)
	}
}

// Where appends a list predicates to the ServiceWithIDMutation builder.
func (m *ServiceWithIDMutation) Where(ps ...predicate.ServiceWithID) {
	m.predicates = append(m.predicates, ps...)
}

// Op returns the operation name.
func (m *ServiceWithIDMutation) Op() Op {
	return m.op
}

// Type returns the node type of this mutation (ServiceWithID).
func (m *ServiceWithIDMutation) Type() string {
	return m.typ
}
...
// order to get all numeric fields that were incremented/decremented, call
// AddedFields().
func (m *ServiceWithIDMutation) Fields() []string {
	fields := make([]string, 0, 0)
	return fields
}
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *ServiceWithIDMutation) Field(name string) (ent.Value, bool) {
	return nil, false
}
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *ServiceWithIDMutation) OldField(ctx context.Context, name string) (ent.Value, error) {
	return nil, fmt.Errorf("unknown ServiceWithID field %s", name)
}

// SetField sets the value of a field with the given name. It returns an error if
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *ServiceWithIDMutation) SetField(name string, value ent.Value) error {
	switch name {
	}
	return fmt.Errorf("unknown ServiceWithID field %s", name)
}

// AddedFields returns all numeric fields that were incremented/decremented during
// this mutation.
func (m *ServiceWithIDMutation) AddedFields() []string {
	return nil
}
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *ServiceWithIDMutation) AddedField(name string) (ent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *ServiceWithIDMutation) AddField(name string, value ent.Value) error {
	return fmt.Errorf("unknown ServiceWithID numeric field %s", name)
}

// ClearedFields returns all nullable fields that were cleared during this
// mutation.
func (m *ServiceWithIDMutation) ClearedFields() []string {
	return nil
}

// FieldCleared returns a boolean indicating if a field with the given name was
// cleared in this mutation.
func (m *ServiceWithIDMutation) FieldCleared(name string) bool {
	_, ok := m.clearedFields[name]
	return ok
}
...
// ClearField clears the value of the field with the given name. It returns an
// error if the field is not defined in the schema.
func (m *ServiceWithIDMutation) ClearField(name string) error {
	return fmt.Errorf("unknown ServiceWithID nullable field %s", name)
}

// ResetField resets all changes in the mutation for the field with the given name.
// It returns an error if the field is not defined in the schema.
func (m *ServiceWithIDMutation) ResetField(name string) error {
	return fmt.Errorf("unknown ServiceWithID field %s", name)
}

// AddedEdges returns all edge names that were set/added in this mutation.
func (m *ServiceWithIDMutation) AddedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *ServiceWithIDMutation) AddedIDs(name string) []ent.Value {
	return nil
}

// RemovedEdges returns all edge names that were removed in this mutation.
func (m *ServiceWithIDMutation) RemovedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *ServiceWithIDMutation) RemovedIDs(name string) []ent.Value {
	return nil
}

// ClearedEdges returns all edge names that were cleared in this mutation.
func (m *ServiceWithIDMutation) ClearedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// EdgeCleared returns a boolean which indicates if the edge with the given name
// was cleared in this mutation.
func (m *ServiceWithIDMutation) EdgeCleared(name string) bool {
	return false
}

// ClearEdge clears the value of the edge with the given name. It returns an error
// if that edge is not defined in the schema.
func (m *ServiceWithIDMutation) ClearEdge(name string) error {
	return fmt.Errorf("unknown ServiceWithID unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *ServiceWithIDMutation) ResetEdge(name string) error {
	return fmt.Errorf("unknown ServiceWithID edge %s", name)
}
