...
func (e *Extension) genWhereInputs() gen.Hook {
	return func(next gen.Generator) gen.Generator {
		...
		inputs := make(map[string]*ast2.Definition)
		return gen.GenerateFunc(func(g *gen.Graph) error {
...
func (e *Extension) updateSchema(inputs map[string]*ast2.Definition) error {
	schema := &ast2.Schema{
		Types: inputs,
	}
...
func (e *Extension) whereType(t *gen.Type) (*ast2.Definition, error) {
	var (
		name    = t.Name + "WhereInput"
		typeDef = &ast2.Definition{
			Name:        name,
			Kind:        ast2.InputObject,
			Description: fmt.Sprintf("%s is used for filtering %s objects.\nInput was generated by ent.", name, t.Name),
			Fields: ast2.FieldList{
				&ast2.FieldDefinition{
					Name: "not",
					Type: ast2.NamedType(name, nil),
				},
			},
		}
	)
	for _, op := range []string{"and", "or"} {
		typeDef.Fields = append(typeDef.Fields, &ast2.FieldDefinition{
			Name: op,
			Type: ast2.ListType(ast2.NonNullNamedType(name, nil), nil),
		})
	}
	...
	for _, e := range edges {
		typeDef.Fields = append(typeDef.Fields,
			&ast2.FieldDefinition{
				...
				Type:        ast2.NamedType("Boolean", nil),
				Description: e.Name + " edge predicates",
			},
			&ast2.FieldDefinition{
				Name: camel("has_" + e.Name + "_with"),
				Type: ast2.ListType(
					ast2.NonNullNamedType(e.Type.Name+"WhereInput", nil),
					nil),
			},
		)
	}
...
func (e *Extension) fieldDefinition(f *gen.Field, op gen.Op) *ast2.FieldDefinition {
	name := camel(f.Name + "_" + op.Name())
	if op == gen.EQ {
		name = camel(f.Name)
	}
	def := &ast2.FieldDefinition{
		Name: name,
		Type: ast2.NamedType(e.mapScalar(f, op), nil),
	}
	if f.Name == "id" {
		def.Type = ast2.NamedType("ID", nil)
	}
	if op.Variadic() {
		def.Type = ast2.ListType(ast2.NonNullNamedType(def.Type.Name(), nil), nil)
	}
	return def
}
