...
func (e *schemaGenerator) buildMutationInputs(t *gen.Type, ant *Annotation, gqlType string) ([]*ast.Definition, error) {
	...
	if ant.MutationInputs.Has(MutationCreate) && !ant.Skip.Is(SkipMutationCreateInput) {
		fields := t.Fields

		name := fmt.Sprintf("Create%sInput", gqlType)
		def := &ast.Definition{
			Name:        name,
			Kind:        ast.InputObject,
			Description: fmt.Sprintf("%s is used for create %s object.\nInput was generated by ent.", name, gqlType),
		}
		for _, f := range fields {
			ant, err := annotation(f.Annotations)
			if err != nil {
			...
			if ant.Skip.Is(SkipMutationCreateInput) || f.IsEdgeField() {
				continue
			}

			scalar := e.mapScalar(f, ant, inputObjectFilter)
			if scalar == "" {
				return nil, fmt.Errorf("%s is not supported as input for %s", f.Name, name)
			}
			def.Fields = append(def.Fields, &ast.FieldDefinition{
				Name:        camel(f.Name),
		...
		for _, e := range t.Edges {
			...
			if ant.Skip.Is(SkipMutationCreateInput) {
				continue
			}

			if e.Unique {
				def.Fields = append(def.Fields, &ast.FieldDefinition{
					Name: camel(e.Name + "_ID"),
					Type: namedType("ID", e.Optional),
				})
			} else {
				def.Fields = append(def.Fields, &ast.FieldDefinition{
					Name: camel(singular(e.Name) + "_IDs"),
					Type: namedType("[ID!]", e.Optional),
				})
			}
		}
	...
	if ant.MutationInputs.Has(MutationUpdate) && !ant.Skip.Is(SkipMutationUpdateInput) {
		fields := t.MutableFields()

		name := fmt.Sprintf("Update%sInput", gqlType)
		def := &ast.Definition{
			Name:        name,
			Kind:        ast.InputObject,
			Description: fmt.Sprintf("%s is used for update %s object.\nInput was generated by ent.", name, gqlType),
		}
		for _, f := range fields {
			ant, err := annotation(f.Annotations)
			if err != nil {
			...
			if ant.Skip.Is(SkipMutationUpdateInput) {
				continue
			}
