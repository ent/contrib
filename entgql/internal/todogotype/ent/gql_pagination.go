...
func WithPetOrder(orders ...*PetOrder) PetPaginateOption {
	var compactedOrder []PetOrder
	for _, order := range orders {
		if order == nil {
			continue
		}
	...
	if len(compactedOrder) == 0 {
		compactedOrder = append(compactedOrder, *DefaultPetOrder)
	}
	return func(pager *petPager) error {
		for _, order := range compactedOrder {
			...
			if order.Field == nil {
				order.Field = DefaultPetOrder.Field
			}
			pager.orders = append(pager.orders, order)
		}
		return nil
...
func newPetPager(opts []PetPaginateOption) (*petPager, error) {
	...
	if len(pager.orders) == 0 {
		pager.orders = append(pager.orders, *DefaultPetOrder)
	}
	return pager, nil
}
...
func (p *petPager) applyCursors(query *PetQuery, after, before *Cursor) *PetQuery {
	if after != nil {
		if values, ok := after.Value.([]any); ok && len(values) == len(p.orders) {
			...
			directions[len(p.orders)] = DefaultPetOrder.Direction
			fields[len(p.orders)] = DefaultPetOrder.Field.field
			...
			var predicate func(string, interface{}) *sql.Predicate
			if DefaultPetOrder.Direction == OrderDirectionAsc {
				predicate = sql.GT
			} else {
				predicate = sql.LT
			}
			...
			query = query.Where(func(s *sql.Selector) {
				s.Where(predicate(s.C(DefaultPetOrder.Field.field), after.ID))
			})
		}
	}
	if before != nil {
		if values, ok := before.Value.([]any); ok && len(values) == len(p.orders) {
			...
			query = query.Where(func(s *sql.Selector) {
				s.Where(predicate(s.C(DefaultPetOrder.Field.field), before.ID))
			})
		}
	}
	return query
...
func (p *petPager) applyOrder(query *PetQuery, reverse bool) *PetQuery {
	...
	for _, order := range p.orders {
		...
		orders = append(orders, direction.orderFunc(order.Field.field))
		if order.Field != DefaultPetOrder.Field {
			addDefault = true
		}
	}
	if addDefault {
		direction := DefaultPetOrder.Direction
		if reverse {
			direction = direction.reverse()
		}
		orders = append(orders, direction.orderFunc(DefaultPetOrder.Field.field))
	}
	query.Order(orders...)
	return query
}
