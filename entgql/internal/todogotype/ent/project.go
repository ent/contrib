...
type Project struct {
	...
	category_projects *bigintgql.BigInt
...
func (*Project) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case project.ForeignKeys[0]: // category_projects
			values[i] = &sql.NullScanner{S: new(bigintgql.BigInt)}
		default:
	...
	for i := range columns {
		...
		case project.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field category_projects", values[i])
			} else if value.Valid {
				pr.category_projects = new(bigintgql.BigInt)
				*pr.category_projects = *value.S.(*bigintgql.BigInt)
			}
		default:
