...
func (b *Bookmark) Todo(ctx context.Context) (*Todo, error) {
	result, err := b.Edges.TodoOrErr()
	if IsNotLoaded(err) {
		result, err = b.QueryTodo().Only(ctx)
	}
	return result, MaskNotFound(err)
}

func (b *Bookmark) Project(ctx context.Context) (*Project, error) {
	result, err := b.Edges.ProjectOrErr()
	if IsNotLoaded(err) {
		result, err = b.QueryProject().Only(ctx)
	}
	return result, MaskNotFound(err)
}
