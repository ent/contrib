...
func (*Todo) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case todo.ForeignKeys[0]: // project_todo
			values[i] = new(sql.NullInt64)
...
func (t *Todo) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case todo.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field project_todo", value)
			} else if value.Valid {
				t.project_todo = new(int)
				*t.project_todo = int(value.Int64)
			}
