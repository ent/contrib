...
func (pr *ProjectQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "todo":
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					pr.loadTotal = append(pr.loadTotal, func(ctx context.Context, nodes []*Project) error {
						...
						var v []struct {
							NodeID int `sql:"project_todo"`
							Count  int `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(project.TodoColumn, ids...))
						})
						if err := query.GroupBy(project.TodoColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[int]int, len(v))
						for i := range v {
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				modify := limitRows(project.TodoColumn, limit, pager.orderExpr())
				query.modifiers = append(query.modifiers, modify)
			} else {
				query = pager.applyOrder(query)
			}
			...
			pr.WithNamedTodo(alias, func(wq *TodoQuery) {
				*wq = *query
			})
		}
	}
