...
func (pu *ProjectUpdate) AddTodo(t ...*Todo) *ProjectUpdate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (pu *ProjectUpdate) ClearTodo() *ProjectUpdate {
...
func (pu *ProjectUpdate) RemoveTodo(t ...*Todo) *ProjectUpdate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (pu *ProjectUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if pu.mutation.TodoCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   project.TodoTable,
			Columns: []string{project.TodoColumn},
			Bidi:    false,
...
// AddTodo adds the "todo" edges to the Todo entity.
func (puo *ProjectUpdateOne) AddTodo(t ...*Todo) *ProjectUpdateOne {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
// ClearTodo clears all "todo" edges to the Todo entity.
func (puo *ProjectUpdateOne) ClearTodo() *ProjectUpdateOne {
...
// RemoveTodo removes "todo" edges to Todo entities.
func (puo *ProjectUpdateOne) RemoveTodo(t ...*Todo) *ProjectUpdateOne {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
