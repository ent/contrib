...
func (pq *ProjectQuery) Order(o ...project.Order) *ProjectQuery {
	pq.order = append(pq.order, o...)
	return pq
}

// QueryTodo chains the current query on the "todo" edge.
func (pq *ProjectQuery) QueryTodo() *TodoQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, project.TodoTable, project.TodoColumn),
		)
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		config:     pq.config,
		ctx:        pq.ctx.Clone(),
		order:      append([]project.Order{}, pq.order...),
...
func (pq *ProjectQuery) WithTodo(opts ...func(*TodoQuery)) *ProjectQuery {
	query := (&TodoClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withTodo; query != nil {
		if err := pq.loadTodo(ctx, query, nodes,
			func(n *Project) { n.Edges.Todo = []*Todo{} },
			func(n *Project, e *Todo) { n.Edges.Todo = append(n.Edges.Todo, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedTodo {
		if err := pq.loadTodo(ctx, query, nodes,
			func(n *Project) { n.appendNamedTodo(name) },
			func(n *Project, e *Todo) { n.appendNamedTodo(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadTodo(ctx context.Context, query *TodoQuery, nodes []*Project, init func(*Project), assign func(*Project, *Todo)) error {
	...
	query.Where(predicate.Todo(func(s *sql.Selector) {
		s.Where(sql.InValues(project.TodoColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_todo
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_todo" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_todo" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProjectQuery) WithNamedTodo(name string, opts ...func(*TodoQuery)) *ProjectQuery {
	query := (&TodoClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedTodo == nil {
		pq.withNamedTodo = make(map[string]*TodoQuery)
	}
	pq.withNamedTodo[name] = query
	return pq
}
