...
func TestNestedConnection(t *testing.T) {
	...
	for i := range bulkU {
		bulkU[i] = ec.User.Create().
			...
			AddGroupIDs(groupIDs(groups[:len(groups)-i])...).
			SetRequiredMetadata(map[string]any{})
	}
	users := ec.User.CreateBulk(bulkU...).SaveX(ctx)
	ec.User.UpdateOne(users[0]).AddFriendIDs(userIDs(users[1:])...).SaveX(ctx) // user 0 is friends with all
	ec.User.UpdateOne(users[1]).AddFriendIDs(userIDs(users[2:])...).SaveX(ctx) // user 1 is friends with all
...
func groupIDs(groups []*ent.Group) []int {
